...
//
void generateEssaiLibreFile()//generateRecapFilePractice
{
	FILE *fichier = NULL;

	//we're opening the file with the argument a because we want to append every practice                 //traduire
	//recap to that file
	fichier = fopen("essaiLibre.txt", "a"); //cree le fichier et l ouvre
	structCar temp = cars[0];
	temp.bestCircuit=10000;//Gros nombre pour que la condition soit correct 
	if(fichier != NULL){
		...
		for(int i = 0; i < 20; i++){
			...
			fprintf(fichier,"|     %2d    |   %9f s  |   %9f s  |   %9f s  |    %9f s   |\n", 						cars[i].number,cars[i].bestS1,cars[i].bestS2,cars[i].bestS3,cars[i].bestCircuit);
		}

		fprintf(fichier,"\n The winner is the car number %2d with a time of %9f s\n",temp.number, temp.bestCircuit);
		fclose(fichier);
	}
	else{
